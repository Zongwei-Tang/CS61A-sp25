...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        count = 0
        while not a.is_hive:
            if a.bees and count >= self.lower_bound and count <= self.upper_bound:
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            for i in self.place.bees[:]:
                Insect.reduce_health(i, amount + self.damage)
        else:
            for i in self.place.bees[:]:
                Insect.reduce_health(i, amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chewing:
            ...
            if self.place.bees:
                a = random_bee(self.place.bees)
                a.reduce_health(a.health)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while a:
            i = a.ant
            if i and i.is_container and i.ant_contained:
                if not i.is_doubled:
                    i.double()
                    i.is_doubled = True
                if not i.ant_contained.is_doubled:
                    i.ant_contained.double()
                    i.ant_contained.is_doubled = True
            elif i:
                if not i.is_doubled:
                    i.double()
                    i.is_doubled = True
...
class SlowThrower(ThrowerAnt):
    ...
    # BEGIN Problem EC 1
    implemented = True 
    damage = 0  # Change to True to view in the GUI
    # END Problem EC 1
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not a.is_hive:
            if a.ant:
                result[a.ant] = distance
                if a.ant.is_container and a.ant.ant_contained:
                    result[a.ant.ant_contained] = distance
            for i in a.bees:
                result[i] = distance
            distance += 1
            a = a.entrance
