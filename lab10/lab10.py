...
def floor_div(args):
    ...
    current = args.rest
    previous = args
    a = calc_eval(previous.first)
    b = calc_eval(current.first)
    result = a // b
    while current.rest != nil:
...
def eval_define(expressions):
    ...
    2
    """
    a = calc_eval(expressions.rest.first)
    bindings[expressions.first] = a
