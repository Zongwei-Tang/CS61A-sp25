...
def boar_brawl(player_score, opponent_score):
    ...
    if opponent_score < 10:
        return max(abs(player_score % 10) * 3, 1)
    else:
        return max(1, abs(opponent_score // 10 % 10 - player_score % 10) * 3)
    # END PROBLEM 2
...
def make_averaged(original_function, times_called=1000):
    ...
    def original(*args):
        ...
        for i in range(times_called):
            sums += original_function(*args)
        return sums / times_called
    return original
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    max_value = average_function(maxi, dice)
    for i in range(2,11):
        currentvalue = average_function(i, dice)
        if currentvalue > max_value:
            maxi = i
            max_value = currentvalue
    return maxi
"""
Used AI to figure the bug here: currentvalue is needed here.
"""
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    if score - opponent_score > 25:
        return 3
    averagevalue6 = make_averaged(roll_dice, 100)
    if averagevalue6(6) < boar_brawl(score, opponent_score):
        return 0
    return 6  
    # END PROBLEM 12
"""I cannot figure out how to let the function know the value goal without adding a new parameter😅
Also, dont set the times_called 1000, the test will timeout.
My final strategy works! I am beaten by my own strategy😭
"""
